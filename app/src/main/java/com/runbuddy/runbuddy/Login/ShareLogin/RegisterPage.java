...
public class RegisterPage extends Activity implements OnClickListener, Callback {
	...
	public final static int INTENT_USR_PICTURE = 4;
	...
	private static boolean shareLogin_success_flag = false;
	private static String use_picturePath;
	...
	public static String getUse_picturePath() {
		return use_picturePath;
	}

	public static void setUse_picturePath(String use_picturePath) {
		RegisterPage.use_picturePath = use_picturePath;
	}
	...
	public void onClick(View v) {
		...
		case R.id.tv_ensure:
			...
			if (registerListener.onRegister(userInfo)) {
				...
				intent.putExtra("logout", "RegisterPage");
				setResult(INTENT_USR_PICTURE,intent);
				finish();
				//Intent intent = new Intent(getApplicationContext(), MainActivity.class);//主界面
				...
				Intent intent = new Intent(getApplicationContext(), LoginActivity.class);//返回登录页面
				startActivity(intent);
			}
			break;
	...
	public static boolean isShareLogin_success_flag() {
		return shareLogin_success_flag;
	}

	public void setShareLogin_success_flag(boolean shareLogin_success_flag) {
		this.shareLogin_success_flag = shareLogin_success_flag;
	}
